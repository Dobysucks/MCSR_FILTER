...
class MCSeedFinderProvider(IMinecraft26WorldGenProvider):
    ...
    def self_test(self) -> ProviderSelfTest:
        ...
        try:
            ...
            # -------------------------------------------------------
            # Exercise the required Nether compound filter:
            # (Stables OR Treasure OR Bridge OR Housing) AND Fortress.
            # -------------------------------------------------------

            nether_filter = FilterConfig(
                village=False,
                shipwreck=False,
                desert_temple=False,
                ruined_portal=False,
            ...
            try:
                ...
                return ProviderSelfTest(
                    ...
                    self.name,
                    BACKEND_PROGRAM_VERSION,
                    f"Nether compound structure lookup failed: {exc}",
                    sha256=digest,
                )

            if not isinstance(nether_probe, dict):
                return ProviderSelfTest(
                    False,
                    self.name,
                    BACKEND_PROGRAM_VERSION,
                    "Nether compound structure lookup returned "
                    "an invalid result.",
                    sha256=digest,
                )
    ...
    def _check(
        ...
        if pool == "overworld":
            ...
            bastion_options: list[dict[str, Any]] = []

            if filters.stables:
                bastion_options.append(
            ...
            if not bastion_options:
                raise GenerationUnavailableError(
                    "No Nether Bastion subtype filters are enabled."
                )
    ...
    def find_batch(
        ...
        """Find matching seeds using MCSeedFinder's native batch search."""
        if not self._verified:
            raise GenerationUnavailableError(
                "Provider self-test has not passed."
            )

        config = self._build_config(pool, filters)
        ...
        try:
            ...
            if result.returncode != 0:
                raise GenerationUnavailableError(
                    result.stderr.strip()
                    or "MCSeedFinder batch search failed."
                )
            ...
            if not reports:
                raise GenerationUnavailableError(
                    "MCSeedFinder batch search returned no results."
                )
    ...
    def _build_config(
        ...
        if pool == "overworld":
                ...
                selected.append(
                    {
                        "type": "all",
                        "conditions": [
                            self._overworld_structure_condition("village", 112),
                            self._overworld_piece_condition(
                                "village",
                                self.VILLAGE_BLACKSMITH_GROUP,
                                112,
                ...
                selected.append(
                    {
                        "type": "all",
                        "conditions": [
                            self._overworld_structure_condition("shipwreck", 64),
                            self._overworld_piece_condition(
                                "shipwreck",
                                self.SHIPWRECK_FULL_GROUP,
                                64,
            ...
            if not selected:
                raise GenerationUnavailableError(
                    "No Overworld structure filters are enabled."
                )
        ...
        if pool != "nether":
            raise GenerationUnavailableError(
                f"Native batch search is not supported for pool: {pool}"
            )
        ...
        if filters.stables:
            bastion_options.append(
                self._piece_condition(self.STABLES_PIECE)
            )

        if filters.treasure:
            bastion_options.append(
                self._piece_condition(self.TREASURE_PIECE)
            )

        if filters.bridge:
            bastion_options.append(
                self._piece_condition(self.BRIDGE_PIECE)
            )
